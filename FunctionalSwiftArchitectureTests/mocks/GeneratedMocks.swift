// MARK: - Mocks generated from file: FunctionalSwiftArchitecture/presentation/navigation/Navigation.swift at 2017-12-12 10:01:16 +0000
...
class MockJokesNavigation: JokesNavigation, Cuckoo.Mock {
    typealias MocksType = JokesNavigation
    typealias Stubbing = __StubbingProxy_JokesNavigation
    typealias Verification = __VerificationProxy_JokesNavigation
    let cuckoo_manager = Cuckoo.MockManager()

    private var observed: JokesNavigation?

    func spy(on victim: JokesNavigation) -> Self {
        observed = victim
        return self
    }
    ...
    // ["name": "go", "returnSignature": " -> Reader<GetCategoriesContext, Void>", "fullyQualifiedName": "go(categoryName: String) -> Reader<GetCategoriesContext, Void>", "parameterSignature": "categoryName: String", "parameterSignatureWithoutNames": "categoryName: String", "inputTypes": "String", "isThrowing": false, "isInit": false, "hasClosureParams": false, "@type": "ClassMethod", "accessibility": "", "parameterNames": "categoryName", "call": "categoryName: categoryName", "parameters": [CuckooGeneratorFramework.MethodParameter(label: Optional("categoryName"), name: "categoryName", type: "String", range: CountableRange(242..<262), nameRange: CountableRange(242..<254))], "returnType": "Reader<GetCategoriesContext, Void>", "isOptional": false, "stubFunction": "Cuckoo.StubFunction"]
     override func go(categoryName: String)  -> Reader<GetCategoriesContext, Void> {
        
            return cuckoo_manager.call("go(categoryName: String) -> Reader<GetCategoriesContext, Void>",
                parameters: (categoryName),
                original: observed.map { o in
                    return { (args) -> Reader<GetCategoriesContext, Void> in
                        let (categoryName) = args
                        return o.go(categoryName: categoryName)
                    }
                })
    ...
    struct __StubbingProxy_JokesNavigation: Cuckoo.StubbingProxy {
        ...
        func go<M1: Cuckoo.Matchable>(categoryName: M1) -> Cuckoo.StubFunction<(String), Reader<GetCategoriesContext, Void>> where M1.MatchedType == String {
            let matchers: [Cuckoo.ParameterMatcher<(String)>] = [wrap(matchable: categoryName) { $0 }]
            return .init(stub: cuckoo_manager.createStub("go(categoryName: String) -> Reader<GetCategoriesContext, Void>", parameterMatchers: matchers))
        }
        
    }


    struct __VerificationProxy_JokesNavigation: Cuckoo.VerificationProxy {
        private let cuckoo_manager: Cuckoo.MockManager
        ...
        @discardableResult
        func go<M1: Cuckoo.Matchable>(categoryName: M1) -> Cuckoo.__DoNotUse<Reader<GetCategoriesContext, Void>> where M1.MatchedType == String {
            let matchers: [Cuckoo.ParameterMatcher<(String)>] = [wrap(matchable: categoryName) { $0 }]
            return cuckoo_manager.verify("go(categoryName: String) -> Reader<GetCategoriesContext, Void>", callMatcher: callMatcher, parameterMatchers: matchers, sourceLocation: sourceLocation)
        }
 ...
 class JokesNavigationStub: JokesNavigation {
     ...
     override func go(categoryName: String)  -> Reader<GetCategoriesContext, Void> {
        return DefaultValueRegistry.defaultValue(for: Reader<GetCategoriesContext, Void>.self)
    }
...
// MARK: - Mocks generated from file: FunctionalSwiftArchitecture/presentation/JokesPresentation.swift at 2017-12-12 10:01:16 +0000
